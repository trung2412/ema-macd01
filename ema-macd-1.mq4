...
void CheckForOpen()   
   {  
      ...
      ema5a  = iMA(Symbol(),Period(),5,0,MODE_EMA,PRICE_CLOSE,1);
      ema15a = iMA(Symbol(),Period(),15,0,MODE_EMA,PRICE_CLOSE,1);
      ...
      if(ema5a>ema15a && ema5<ema15 && macd<0 && Bid<ema50 && Bid<ema100)
        {
            order = OrderSend(Symbol(),OP_SELL,0.1,Bid,5,ema50,Bid - TP*Point,"",MAGICMA,0,Red);
            return;
        } 
      if(ema5a<ema15a && ema5>ema15 && macd>0 && Bid>ema50 && Bid>ema100)
        {
            order = OrderSend(Symbol(),OP_BUY,0.1,Ask,5,ema50,Ask + TP*Point,"",MAGICMA,0,Green);
            return;
        }
   }
